...
export default function EditService() {
  ...
  // Fetch the service data and categories
  const fetchServiceData = async () => {
    ...
    try {
      ...
      if (!serviceData) {
        ...
        navigate('/all-services-products')
        return
      }
      ...
      setFormData({
        businessName: serviceData.business_name || '',
        category: serviceData.categories_ids[0] || 'default',
        description: serviceData.description || '',
        isContactShared: serviceData.contact_info?.show || false,
        isIdShared: serviceData.id_info?.show || false,
        contactInfo: {
          contact_number: serviceData.contact_info?.contact_number || '',
          show: serviceData.contact_info?.show || false,
        },
        idInfo: {
          id_number: serviceData.id_info?.id_number || '',
          show: serviceData.id_info?.show || false,
        },
        whatsapp_no: serviceData.whatsapp_no || '',
      })

      setIdImageLink(serviceData.id_image || '')

      // Set location info
      setLocationInfo({
        address: serviceData.location_info?.address || '',
        lat: serviceData.location_info?.lat || null,
        lng: serviceData.location_info?.lng || null,
      })

      // Set tags/keywords
      setTags(serviceData.keyword_list || [])

      // Set media URLs
      if (serviceData.images && serviceData.images.length > 0) {
        setImagesUrls(
          serviceData.images.map((url) => ({
            url,
      ...
      if (serviceData.audios && serviceData.audios.length > 0) {
        setAudioUrls(
          serviceData.audios.map((url) => ({
            url,
            path: extractPathFromUrl(url),
            name: extractNameFromUrl(url),
          }))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        uid: service.uid || localStorage.getItem('userId'),
      ...
      // Update the service_location document
      const locationPayload = {
        created_at: service.created_at,
        updated_at: new Date().toISOString(),
        created_by: service.uid || localStorage.getItem('userId'),
        id: serviceId,
      ...
      await updateDocument('services_locations', serviceId, locationPayload)
          ...
          <div className="col-md-12 col-lg-12 col-xl-12 p-4">
            <form onSubmit={handleSubmit}>
              <div className="divider mb-4 pb-2">
                <h1 className="fw-bold mb-0">Edit Service</h1>
              </div>
              ...
              <div className="form-outline mb-4">
                <label className="form-label">Serivce Title</label>
                <input
                  type="text"
              ...
              <div className="text-center text-lg-start mt-4 pt-2">
                ...
                <button
                  ...
                  onClick={() => navigate('/all-services-products')}
                >
                  Cancel
                </button>
              </div>
